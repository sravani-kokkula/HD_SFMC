 ...
 */
	...
	function __construct($getWSDL = false, $debug = false, $params = null) 
	{
		...
		$this->xmlLoc = $rootPath.'/vendor/salesforce-mc/fuel-sdk-php/ExactTargetWSDL.xml';
               
                $scopeConfig = $objectManager->get('\Magento\Framework\App\Config\ScopeConfigInterface');
$clientid=$scopeConfig->getValue('sfmc_email_settings/sfmc_enabled_settings/i95dev_sfmc_clientid', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
$clientsecretkey=$scopeConfig->getValue('sfmc_email_settings/sfmc_enabled_settings/i95dev_sfmc_clientsecretkey', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);

		//if (file_exists(realpath("config.php")))
			//$config = include 'config.php';
	$config = include $rootPath.'/vendor/salesforce-mc/fuel-sdk-php/config.php';
		if ($config)
		{
			$this->wsdlLoc = $config['defaultwsdl'];
                         //$config['clientid'];//$config['clientsecret'];
			$this->clientId = $clientid;
			$this->clientSecret = $clientsecretkey; 
		...
		if ($params) 
		{
			...
			if ($params && array_key_exists('clientid', $params)){$this->clientId = $clientid; }
			if ($params && array_key_exists('clientsecret', $params)){$this->clientSecret = $clientsecretkey;}
